...
def addCourse():
    listOfPrograms = Program.query.all()
    if request.method=="POST":
        coursename = request.form.get("coursename")
        description = request.form.get("description")
        programID = request.form.get("programID")
        print(f"C={coursename} D={description} P={programID}")
        newCourse  = Course.add(name=coursename, description=description, programID=programID)
        flash(f"Added! {newCourse.id}")

    return render_template("addCourse.html", listOfPrograms=listOfPrograms)
